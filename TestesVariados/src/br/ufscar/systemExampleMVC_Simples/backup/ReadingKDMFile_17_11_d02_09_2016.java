package br.ufscar.systemExampleMVC_Simples.backup;
...
public class ReadingKDMFile_17_11_d02_09_2016 {
...
//		save(segment, kdmPath);
	...
	private boolean verifyIfRelationContaisLayer (KDMRelationship relationToVerify, ArrayList<Layer> allLayers) {
		...
		ClassUnit[] classUnitToAndFrom = getOriginAndDestinyClassUnit(relationToVerify.getTo(), relationToVerify.getFrom());
		boolean to = false, from = false;
		
		if (packageToAndFrom[0] == null || packageToAndFrom[1] == null) {
			if (classUnitToAndFrom[0] == null || classUnitToAndFrom[1] == null) {
				return false;
			}
			for (Layer layer1 : allLayers) {
				
				if (mappingLayerToClass(layer1, classUnitToAndFrom[0]))
					to = true;
				
				if (mappingLayerToClass(layer1, classUnitToAndFrom[1]))
					from = true;									 					
	...
	private boolean mappingLayerToClass(Layer layer, ClassUnit classUnitToVerify) {
		String pathToVerifyClass = getPathOfClass(classUnitToVerify, "");
		...
		for (KDMEntity kdmEntity : allImplementation) {
			...
			if (kdmEntity instanceof ClassUnit) {
				...
				pathToVerifyLayer = getPathOfClass((ClassUnit) kdmEntity,
						pathToVerifyLayer);

			}
			System.out.println("mappingLayerToClass ---------> " + pathToVerifyLayer + " == " + pathToVerifyClass);
			if (pathToVerifyLayer.equals(pathToVerifyClass)) {
	...
	private String getPathOfClass(EObject classUnitToGetThePath, String pathToGet) {
		
		if (classUnitToGetThePath instanceof ClassUnit) {

			ClassUnit classUnitToGet = (ClassUnit) classUnitToGetThePath;
			
			pathToGet = getPathOfClass(
					(EObject) classUnitToGetThePath.eContainer(), pathToGet);

			if (classUnitToGetThePath.eContainer() instanceof CodeModel) {
				pathToGet += classUnitToGet.getName();
			} else {
	...
	public void createAggreatedRelationShips(ArrayList<Layer> layers,
		...
		for (KDMRelationship relation : allRelations) {
			...
			ClassUnit[] classUnitToAndFrom = null;
			...
			*/
			...
			classUnitToAndFrom = getOriginAndDestinyClassUnit(to, from);
			if(packageToAndFrom[0] == null)
				System.err.println("createAggreatedRelationShips>PT " + "null");
			else
				System.err.println("createAggreatedRelationShips>PT " + packageToAndFrom[0].getName());
			if(packageToAndFrom[1] == null)
				System.err.println("createAggreatedRelationShips>PF " + "null");
			else
				System.err.println("createAggreatedRelationShips>PF " + packageToAndFrom[1].getName());
			
			if(classUnitToAndFrom[0] == null)
				System.err.println("createAggreatedRelationShips>CT " + "null");
			else
				System.err.println("createAggreatedRelationShips>CT " + classUnitToAndFrom[0].getName());
			if(classUnitToAndFrom[1] == null)
				System.err.println("createAggreatedRelationShips>CF " + "null");
			else
				System.err.println("createAggreatedRelationShips>CF " + classUnitToAndFrom[1].getName());
			...
			if (!packageToAndFrom[0].getName().equals(packageToAndFrom[1].getName()) 
					||
				!classUnitToAndFrom[0].getName().equals(classUnitToAndFrom[1].getName())
				) {
				for (Layer layers1 : layers) {
					
					//Itera nas layers ate encontrar a layer que corresponde a origem(from) da chamada(call) 
					if (mappingLayerToPackage(layers1, packageToAndFrom[1]) || mappingLayerToClass(layers1, classUnitToAndFrom[1])
							) {
						...
						if (aggregatedRelationship.size() > 0) {
							...
							for (int i = 0; i < aggregatedRelationship.size(); i++) {
								...
								if (mappingLayerToPackage((Layer) aggregatedRelationship.get(i).getTo(), packageToAndFrom[0])
										|| mappingLayerToClass((Layer) aggregatedRelationship.get(i).getTo(), classUnitToAndFrom[0])
										) {
	...
	private ClassUnit[] getOriginAndDestinyClassUnit(EObject to, EObject from) {
		ClassUnit[] classUnitToAndFrom = new ClassUnit[2];
		ClassUnit auxTo = null;
		ClassUnit auxFrom = null;
		...
		auxTo = getToOrFromClassUnit(to, auxTo);
		auxFrom = getToOrFromClassUnit(from, auxFrom);
		classUnitToAndFrom[0] = auxTo;
		classUnitToAndFrom[1] = auxFrom;

		return classUnitToAndFrom;

	}



	private ClassUnit getToOrFromClassUnit(EObject element, ClassUnit toOrFrom) {
		if (element instanceof ClassUnit) {

			return (ClassUnit) element;
			...
			toOrFrom = getToOrFromClassUnit(element.eContainer(), toOrFrom);
